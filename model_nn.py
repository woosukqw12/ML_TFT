...
class InnerNode:
    ...
    def get_penalty(self):
        penalty = (
            torch.sum(self.prob * self.path_prob) / torch.sum(self.path_prob),
            self.lmbda,
        )
...
class SoftDecisionTree(nn.Module):
    def __init__(self, args):
        ...
        self.optimizer = optim.SGD(
            self.parameters(), lr=self.args.lr, momentum=self.args.momentum
        )
        self.test_acc = []
    ...
    def cal_loss(self, x, y):
        ...
        for (path_prob, Q) in leaf_accumulator:
            TQ = torch.bmm(
                y.view(batch_size, 1, self.args.output_dim),
                torch.log(Q).view(batch_size, self.args.output_dim, 1),
            ).view(-1, 1)
            loss += path_prob * TQ
        ...
        return (
            -loss + C,
            output,
        )  ## -log(loss) will always output non, because loss is always below zero. I suspect this is the mistake of the paper?
    ...
    def train_(self, train_loader, epoch):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            data = Variable(data)
            if (
                not batch_size == self.args.batch_size
